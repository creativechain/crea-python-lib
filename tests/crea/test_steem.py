...
class Testcases(unittest.TestCase):
    ...
    def test_transfer(self):
        ...
        tx = acc.transfer(
            "test", 1.33, "CBD", memo="Foobar", account="test1")
        self.assertEqual(
            tx["operations"][0][0],
            "transfer"
        )
    ...
    def test_create_account(self):
        bts = Crea(node=self.nodelist.get_nodes(),
                    nobroadcast=True,
                    unsigned=True,
                    data_refresh_time_seconds=900,
                    keys={"active": wif, "owner": wif, "memo": wif},
    ...
    def test_finalizeOps(self):
        ...
        acc.transfer("test1", 1, "CREA", append_to=tx1)
        acc.transfer("test1", 2, "CREA", append_to=tx2)
        acc.transfer("test1", 3, "CREA", append_to=tx1)
        tx1 = tx1.json()
        tx2 = tx2.json()
        ops1 = tx1["operations"]
        ops2 = tx2["operations"]
    ...
    def test_update_memo_key(self):
        ...
        self.assertEqual(acc.crea.prefix, prefix)
        acc.crea.txbuffer.clear()
    ...
    def test_post(self):
        ...
        self.assertEqual(json_metadata["app"], "crea/%s" % (crea_version))
        self.assertEqual(
            (tx["operations"][1][0]),
            "comment_options"
        )
    ...
    def test_comment_option(self):
        ...
        self.assertIn(
            "gtg",
            op["author"])
        self.assertEqual('1000000.000 CBD', op["max_accepted_payout"])
        self.assertEqual(10000, op["percent_crea_dollars"])
    ...
    def test_sbd_to_rshares(self):
        ...
        for v in test_values:
            ...
            except ValueError:  # Reward pool smaller than 1e7 CBD (e.g. caused by a very low crea price)
                continue
    ...
    def test_rshares_to_vote_pct(self):
        ...
        for vote_pct in range(500, 10000, 500):
            ...
            vote_pct_ret = stm.rshares_to_vote_pct(rshares, crea_power=sp, voting_power=voting_power)
