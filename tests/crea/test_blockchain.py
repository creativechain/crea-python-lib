...
from crea.exceptions import BlockWaitTimeExceeded
...
class Testcases(unittest.TestCase):
    ...
    def test_blockchain(self):
        bts = self.bts
        b = Blockchain(crea_instance=bts)
        num = b.get_current_block_num()
    ...
    def test_estimate_block_num(self):
        ...
        old_block = Block(num - 60, crea_instance=bts)
        date = old_block.time()
    ...
    def test_wait_for_and_get_block(self):
        bts = self.bts
        b = Blockchain(crea_instance=bts, max_block_wait_repetition=18)
        ...
        b2 = Blockchain(crea_instance=bts, max_block_wait_repetition=1)
        with self.assertRaises(
    ...
    def test_hash_op(self):
        bts = self.bts
        b = Blockchain(crea_instance=bts)
        op1 = {'type': 'vote_operation', 'value': {'voter': 'ubg', 'author': 'yesslife', 'permlink': 'creait-sandwich-contest-week-25-2da-entry', 'weight': 100}}
        op2 = ['vote', {'voter': 'ubg', 'author': 'yesslife', 'permlink': 'creait-sandwich-contest-week-25-2da-entry', 'weight': 100}]
        hash1 = b.hash_op(op1)
        hash2 = b.hash_op(op2)
        self.assertEqual(hash1, hash2)

    def test_signing_appbase(self):
        b = Blockchain(crea_instance=self.bts)
        st = None
