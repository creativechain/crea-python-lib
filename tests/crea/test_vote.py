...
from crea.vote import Vote, ActiveVotes, AccountVotes
from crea.instance import set_shared_crea_instance
from crea.utils import construct_authorperm, resolve_authorperm, resolve_authorpermvoter, construct_authorpermvoter
...
class Testcases(unittest.TestCase):
    ...
    def test_vote(self):
        bts = self.bts
        vote = Vote(self.authorpermvoter, crea_instance=bts)
        ...
        vote = Vote(self.voter, authorperm=self.authorperm, crea_instance=bts)
        self.assertEqual(self.voter, vote["voter"])
        self.assertEqual(self.author, vote["author"])
        self.assertEqual(self.permlink, vote["permlink"])
        vote_json = vote.json()
    ...
    def test_keyerror(self):
        ...
        with self.assertRaises(
            ...
            Vote(construct_authorpermvoter(self.author, self.permlink, "asdfsldfjlasd"), crea_instance=bts)

        with self.assertRaises(
            exceptions.VoteDoesNotExistsException
        ):
            Vote(construct_authorpermvoter(self.author, "sdlfjd", "asdfsldfjlasd"), crea_instance=bts)

        with self.assertRaises(
            exceptions.VoteDoesNotExistsException
        ):
            Vote(construct_authorpermvoter("sdalfj", "dsfa", "asdfsldfjlasd"), crea_instance=bts)

    def test_activevotes(self):
        bts = self.bts
        votes = ActiveVotes(self.authorperm, crea_instance=bts)
        votes.printAsTable()
    ...
    def test_accountvotes(self):
        ...
        votes = AccountVotes(self.author, start=limit_time, crea_instance=bts)
