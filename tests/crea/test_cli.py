...
from crea.cli import cli, balance
...
class Testcases(unittest.TestCase):
    ...
    def test_balance(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['balance', 'creabot', 'creapy'])
        self.assertEqual(result.exit_code, 0)

    def test_interest(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'interest', 'creabot', 'creapy'])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_info(self):
        ...
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['info', 'creabot'])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_upvote(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'upvote', '@creait/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'upvote', '@creait/firstpost', '100'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'upvote', '--weight', '100', '@creait/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_downvote(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'downvote', '@creait/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'downvote', '@creait/firstpost', '100'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'downvote', '--weight', '100', '@creait/firstpost'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_transfer(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'transfer', 'creabot', '1', 'CBD', 'test'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_powerdownroute(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'powerdownroute', 'creabot'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_permissions(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['permissions', 'creabot'])
        self.assertEqual(result.exit_code, 0)

    def test_follower(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['follower', 'creabot'])
        self.assertEqual(result.exit_code, 0)

    def test_following(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['following', 'creabot'])
        self.assertEqual(result.exit_code, 0)

    def test_muter(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['muter', 'creabot'])
        self.assertEqual(result.exit_code, 0)

    def test_muting(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['muting', 'crea'])
        self.assertEqual(result.exit_code, 0)

    def test_allow_disallow(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'allow', '--account', 'creabot', '--permission', 'posting', 'creapy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'disallow', '--account', 'creabot', '--permission', 'posting', 'rewarding'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_approvewitness(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'approvewitness', '-a', 'creapy', 'holger80'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_disapprovewitness(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'disapprovewitness',  '-a', 'creapy', 'holger80'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_newaccount(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'newaccount', 'crea3'], input="test\ntest\ntest\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', 'newaccount', 'crea3'], input="test\ntest\ntest\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_importaccount(self):
        ...
        result = runner.invoke(cli, ['importaccount', '--roles', '["owner", "active", "posting", "memo"]', 'crea2'], input="test\numybjvCafrt8LdoCjEimQiQ4\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_buy(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', '-x', 'buy', '1', 'CREA', '2.2'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'buy', '1', 'CREA'], input="y\ntest\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'buy', '1', 'CBD', '2.2'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'buy', '1', 'CBD'], input="y\ntest\n")
        self.assertEqual(result.exit_code, 0)

    def test_sell(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', '-x', 'sell', '1', 'CREA', '2.2'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'sell', '1', 'CBD', '2.2'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'sell', '1', 'CREA'], input="y\ntest\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-ds', '-x', 'sell', '1', 'CBD'], input="y\ntest\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_recrea(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dso', 'recrea', '@creait/firstposte'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_follow_unfollow(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dso', 'follow', 'creapy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dso', 'unfollow', 'creapy'], input="test\n")
        self.assertEqual(result.exit_code, 0)

    def test_mute_unmute(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-dso', 'mute', 'creapy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
        result = runner.invoke(cli, ['-dso', 'unfollow', 'creapy'], input="test\n")
        self.assertEqual(result.exit_code, 0)
    ...
    def test_witnesscreate(self):
        runner = CliRunner()
        result = runner.invoke(cli, ['-ds', 'witnesscreate', 'crea', pub_key], input="test\n")
    ...
    def test_claimreward(self):
        ...
        result = runner.invoke(cli, ['-ds', 'claimreward', '--claim_all_crea'], input="test\n")
