...
class Testcases(unittest.TestCase):
    ...
    def test_comment(self):
        ...
        with self.assertRaises(
            ...
            Comment("@abcdef/abcdef", crea_instance=bts)
        title = ''
        cnt = 0
        while title == '' and cnt < 5:
            c = Comment(self.authorperm, crea_instance=bts)
            title = c.title
            cnt += 1
            if title == '':
                c.crea.rpc.next()
    ...
    def test_comment_dict(self):
        ...
        cnt = 0
        while title == '' and cnt < 5:
            c = Comment({'author': self.author, 'permlink': self.permlink}, crea_instance=bts)
            c.refresh()
            title = c.title
            cnt += 1
            if title == '':
    ...
    def test_recent_replies(self):
        bts = self.bts
        r = RecentReplies(self.author, skip_own=True, crea_instance=bts)
        self.assertTrue(len(r) > 0)
        self.assertTrue(r[0] is not None)

    def test_recent_by_path(self):
        bts = self.bts
        r = RecentByPath(path="hot", crea_instance=bts)
        self.assertTrue(len(r) > 0)
        self.assertTrue(r[0] is not None)
