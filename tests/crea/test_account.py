...
from crea.utils import formatTimeString
...
class Testcases(unittest.TestCase):
    ...
    def test_history_block_num(self):
        stm = self.bts
        zero_element = 0
        account = Account("fullnodeupdate", crea_instance=stm)
        h_all_raw = []
        for h in account.history_reverse(raw_output=True):
            h_all_raw.append(h)
        h_list = []
    ...
    def test_account_props(self):
        ...
        sp = account.get_crea_power()
        self.assertTrue(sp >= 0)
        vv = account.get_voting_value_CBD()
    ...
    def test_transfer_from_savings(self):
        w = self.account
        w.crea.txbuffer.clear()
        tx = w.transfer_from_savings(1, "CREA", "")
        self.assertEqual(
            (tx["operations"][0][0]),
    ...
    def test_transfer_to_savings(self):
        w = self.account
        w.crea.txbuffer.clear()
        tx = w.transfer_to_savings(1, "CREA", "")
        self.assertEqual(
            (tx["operations"][0][0]),
            "transfer_to_savings"
        )
    ...
    def test_transfer_to_vesting(self):
        w = self.account
        w.crea.txbuffer.clear()
        tx = w.transfer_to_vesting("1 CREA")
        self.assertEqual(
            (tx["operations"][0][0]),
            "transfer_to_vesting"
        )
    ...
    def test_get_vote_pct_for_CBD(self):
        account = self.account
        for vote_pwr in range(5, 100, 5):
            self.assertTrue(9900 <= account.get_vote_pct_for_CBD(account.get_voting_value_CBD(voting_power=vote_pwr), voting_power=vote_pwr) <= 11000)
