...
from creaapi.exceptions import NumRetriesReached, CallRetriesReached
...
class Testcases(unittest.TestCase):
    ...
    def test_connect_test_node(self):
        rpc = self.rpc
        self.assertIn(rpc.url, self.nodes + self.nodes_creait)
        rpc.rpcclose()
        rpc.rpcconnect()
        self.assertIn(rpc.url, self.nodes + self.nodes_creait)
    ...
    def test_num_retries(self):
        with self.assertRaises(
            ...
            CreaNodeRPC(urls="https://wrong.link.com", num_retries=2, timeout=1)
        with self.assertRaises(
            NumRetriesReached
        ):
            CreaNodeRPC(urls="https://wrong.link.com", num_retries=3, num_retries_call=3, timeout=1)
        ...
        with self.assertRaises(
            ...
            CreaNodeRPC(urls=nodes, num_retries=0, num_retries_call=0, timeout=1)

    def test_error_handling(self):
        rpc = CreaNodeRPC(urls=self.nodes_creait, num_retries=2, num_retries_call=3)
        with self.assertRaises(
            exceptions.NoMethodWithName
        ):
