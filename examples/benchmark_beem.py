...
if __name__ == "__main__":
    ...
    nodes = NodeList()
    if node_setup == 0:
        stm = Crea(node=nodes.get_nodes(normal=True, wss=True), num_retries=10)
        max_batch_size = None
        threading = False
        thread_num = 8
    elif node_setup == 1:
        ...
        stm = Crea(node=nodes.get_nodes(appbase=False, https=False), num_retries=10)
        max_batch_size = None
        threading = True
        thread_num = 16
    ...
    for entry in blockchain.blocks(start=last_block_id, max_batch_size=max_batch_size, threading=threading, thread_num=thread_num, thread_limit=1200):
        ...
        if block_no != last_block_id:
            ...
            if last_block_id % 100 == 0:
                ...
                avtran = total_transaction / (last_block_id - 19273700)
                ltime = now
                if last_node != blockchain.crea.rpc.url:
                    last_node = blockchain.crea.rpc.url
                    print("Current node:", last_node)
